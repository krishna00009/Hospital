app.controller('providersController',
    function($scope, $http, $location, $filter, Notification, ngTableParams,  $timeout, $window, $rootScope) {
        ...
        $scope.EditProviders = function (data) {
            $scope.id= data.id;
            $scope.services = data.serviceName;
        ...
        $scope.SaveProviders = function (flagName) {
            if($scope.services == ""||$scope.services ==null||angular.isUndefined($scope.services)){
                Notification.error({message: 'Please Select Services', positionX: 'center', delay: 2000});
            }
            else if (angular.isUndefined($scope.firstName) || $scope.firstName == ''|| $scope.firstName ==null) {

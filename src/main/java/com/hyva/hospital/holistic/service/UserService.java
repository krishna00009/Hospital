...
public class UserService {
 ...
 UOMRepository uomRepository;
    ...
    public UserPojo getloginStatus(){
        Users user = userRepository.findAllByLoginStatus( "Active");
        UserPojo userPojo1 = new UserPojo();
        if(user.getUsername()!=null) {
            userPojo1.setUsername(user.getUsername());
        }
        userPojo1.setId(user.getId());
        userPojo1.setAge( user.getAge());
        userPojo1.setGender( user.getGender());
        userPojo1.setAddress( user.getAddress());
        return userPojo1;
    }


    public UserPojo userValidate(UserPojo userPojo) {
        Users user = userRepository.findByPhoneNumberOrUsernameAndPassword(userPojo.getPhoneNumber(),userPojo.getPhoneNumber(), userPojo.getPassword());
        if (user != null) {
            UserPojo userPojo1 = new UserPojo();
            userPojo1.setUsername( user.getUsername());
            userPojo1.setId(user.getId());
            ...
            userPojo1.setGender(user.getGender());
            userPojo1.setAddress(user.getAddress());
            userPojo1.setPhoneNumber( user.getPhoneNumber());
            userPojo1.setEmail(user.getEmail());
            List<Users> userslist=userRepository.findAllByIdNot(user.getId());
            for(Users users:userslist){
                users.setLoginStatus("inactive");
    ...
    public BasePojo getPaginatedUsersList(BasePojo basePojo,String searchText) {
        ...
        if(basePojo.isLastPage()==true){
            ...
            if (!StringUtils.isEmpty(searchText)) {
                list1 = userRepository.findAllByUsernameContaining(searchText);
            }else {
        ...
        if (!StringUtils.isEmpty(searchText)) {
            users=userRepository.findFirstByUsernameContaining(searchText,sort);
            list = userRepository.findAllByUsernameContaining(searchText,pageable);
        } else {
            users=userRepository.findFirstBy(sort);
            list = userRepository.findAllBy(pageable);
        }
        ...
        List<UserPojo> typesList =UserMapper.mapUsersEntityToPojo(list);
        basePojo=calculatePagination(basePojo,typesList.size());
        basePojo.setList(typesList);
        return basePojo;
    }

        public Users submituserdetails(UserPojo userPojo, String type) throws Exception {
        Users users = new Users();
        if (StringUtils.equalsIgnoreCase( type, "otp" )) {
            if (users.getId() != null) {
                users = userRepository.findByPhoneNumberAndIdNotIn( userPojo.getPhoneNumber(), userPojo.getId() );
            } else {
                users = userRepository.findByPhoneNumber( userPojo.getPhoneNumber() );
            }
            if (users != null) {
                users = null;
            } else {
                users = UserMapper.mapPojoToEntity( userPojo );
                Roles roles =roleRepository.findAllByName( "Patient");
                users.setIdRoles(roles);
                userRepository.save( users );
                if (StringUtils.equalsIgnoreCase( type, "otp" )) {
                    smsService.sendOtp( userPojo.getUsername(), userPojo.getPhoneNumber() );
                }
            }
        } else {
            users = userRepository.findByPhoneNumber( userPojo.getPhoneNumber() );
...
//    }
    ...
    public Customers getPatientByName(String name) {
        Customers customers =new Customers();
           customers = customerRepository.findByFirstNameOrPhoneNumberOrUhid(name,name,name).get(0);
        return customers;
    }
...
//    }
    ...
    public Patient savePatient(patientPojo details) throws JSONException {
        ...
        Appointments appointments=appointmentRepository.findById(details.getAppointmentId());
        Patient patient1=patientRepository.findByAppointmentsId(appointments);
        if(patient1!=null) {
            details.setId( patient1.getId() );
        }
        if (details.getId() != null) {
...
//                Customers customers = customerRepository.findByFirstName(details.getC)
                ...
                patient.setAppointmentsId(appointments);
    ...
    public Patient getPatient(Long appointment){
        Appointments appointments =appointmentRepository.findById(appointment);
        Patient patient = patientRepository.findByAppointmentsId(appointments);
        return patient;
    }
...
//    }
    ...
    public UserPojo SaveCurrentUser(UserPojo userPojo){
        List<Users>list=new ArrayList<>();
        if(userPojo.getId()!=null){
            list=userRepository.findByUsernameAndIdNotIn(userPojo.getUsername(), userPojo.getId());
        }
        else {
            list=userRepository.findAllByUsername(userPojo.getUsername());
        }
        if(list.size()==0){
            Users users = UserMapper.mapPojoToEntity(userPojo);
           Roles roles =roleRepository.findAllByName( userPojo.getRoles());
           users.setIdRoles(roles);
            userRepository.save(users);
            return userPojo;
        }else{
...
//                }
 ...
 public Appointments SaveAppointment(AppointmentPojo details,Long userId) throws JSONException {
    if(details.getId()==null||details.getId()==0) {
        ...
        customerRepository.save(customers);
//        Services services = servicesRepository.findByName(details.getServiceName());
//    Users usersList = userRepository.findByFlagAndFlagType(2,"Providers");
//        details.setServices(services);
//    details.setServicesProviders(usersList);
        ...
        if (details.getId() != null) {
            ...
            if (appointments != null) {
                ...
                appointments.getCustomers().setUserId( userId );
...
//        Customers customers=customerRepository.findById( details.getId() );
...
//        customers.setNotes( details.getNotes() );
    ...
    public Appointments saveOldAppointments(AppointmentPojo appointmentPojo,Long userid){
        Appointments appointments = null;
        Appointments appointments1 = new Appointments();
...
//       grade = (List<GradeMaster>) bsGrademasterRepository.findAll();
    ...
    public List<AppointmentPojo> getAppointmentreport(Date fromdate,Date todate) {
        List<Appointments> appointments = new ArrayList<>();
        appointments =appointmentRepository.findAllByBookDatetimeBetween(fromdate, todate );
...
//       grade = (List<GradeMaster>) bsGrademasterRepository.findAll();
        serviceCategories = (List<ServiceCategories>) categoriesRepository.findAll(sort);
...
//    }
    ...
    public List<CustomersPojo> customerList(Long userid) {
        List<Customers> customer = new ArrayList<>();
        if(userid!=1L) {
            customer = customerRepository.findAllByUserId(userid);
        }else {
        ...
        for(CustomersPojo customers:gdPojoList) {
            ...
            List<Appointments> list =new ArrayList<>( );
            list= appointmentRepository.findAllByCustomers(customers1);
            if(list.size()>0) {
                Appointments appointments = list.get( 0 );
    ...
    public List<ServicesPojo> servicesList() {
        ...
        if (gdPojoList != null) {
            for (ServicesPojo studentPojo : gdPojoList) {
                if(studentPojo.getIdservicecategories()!=null) {
                    ServiceCategories categories = categoriesRepository.findByName( studentPojo.getIdservicecategories().getName() );
                    if (categories != null) {
                        studentPojo.setIdservicecategories( categories );
...
//            details.setId_services(ea_services);
    ...
    public List<ProvidersPojo> getprovidersbasedonservice(String serviceName){
        Sort sort =new Sort( new Sort.Order( Sort.Direction.ASC,"serviceName" ) );
        List<ServicesProviders> eaUsers = new ArrayList<>();

        eaUsers = (List<ServicesProviders>) providersRepository.findAllByServiceName(serviceName,sort);

        List<ProvidersPojo> gdPojoList = ProvidersMapper.mapEntityToPojo(eaUsers);
        return gdPojoList;
    }

    public List<ServicesPojo>getservicebasedoncategories(String name){
        Sort sort=new Sort( new Sort.Order( Sort.Direction.ASC,"name" ) );
        ServiceCategories serviceCategories= categoriesRepository.findAllByName( name );
        List<Services> services =(List<Services>)servicesRepository.findAllByIdservicecategories( serviceCategories,sort );
        List<ServicesPojo> servicesPojos=ServiceMapper.mapServiceEntityToPojo(services);
    ...
    public Services SaveServices(ServicesPojo details) throws JSONException {
        ...
        if (details.getId() != null) {
            ...
            if (services != null) {
                ...
                details.setIdservicecategories(cateobj);
...
//    }
    ...
    public List<UserPojo> getUserList() {
        ...
        List<UserPojo> usersPojo1 =UserMapper.mapUsersEntityToPojo(users);
        return usersPojo1;
    }
    public UserPojo verifyotpdetails(String phoneno,String otp) {
        OtpGeneration otpGeneration =otpRepository.findByPhoneNumber(phoneno );
        if(StringUtils.equalsIgnoreCase( otp,otpGeneration.getOtp() )){
...
//            if(appointments.getCustomers()!=null) {
//                customerRepository.delete( appointments.getCustomers().getId() );
//            }
        }
    }
    ...
    public BasePojo getPaginatedServicesList(BasePojo basePojo,String searchText) {
        ...
        if(basePojo.isLastPage()==true){
            ...
            if (!StringUtils.isEmpty(searchText)) {
                list1 = servicesRepository.findAllByNameContaining(searchText,sort);
            }else {
    ...
    public BasePojo getPaginatedAppointmentList(BasePojo basePojo,String searchText,Long userid) {
       List<Customers> customers=customerRepository.findAllByFirstNameContainingOrPhoneNumberContainingOrUhidContaining(searchText,searchText,searchText);
        ...
        if(basePojo.isLastPage()==true){
            ...
            if (!StringUtils.isEmpty(searchText)&&userid!=1L) {
                list1 = appointmentRepository.findAllByCustomersInAndUserId(customers,userid);
            }else if(!StringUtils.isEmpty(searchText)&&userid==1L) {
                list1 = appointmentRepository.findAllByCustomersIn(customers);
            }else if(StringUtils.isEmpty(searchText)&&userid!=1L) {
                list1 = appointmentRepository.findAllByUserId(userid);
            }else {
        ...
        if (!StringUtils.isEmpty(searchText)&&userid!=1L) {
            if(customers.size()>0) {
                appointments = appointmentRepository.findFirstByCustomersInAndUserId( customers.get( 0 ),userid, sort );
                list = appointmentRepository.findAllByCustomersInAndUserId( customers, pageable,userid );
            }
        }else if(!StringUtils.isEmpty(searchText)&&userid==1L) {
            if(customers.size()>0) {
            ...
            appointments=appointmentRepository.findFirstByUserId(sort,userid);
            list = appointmentRepository.findAllByUserId(pageable,userid);
        }else {
...
//                row.createCell(2).setCellValue(currency.getCurrencyDescription());
                row.createCell(3).setCellValue(currency.getCurrencySymbol());
            }
...
//        table.addCell(pc4);
        ...
        for (ServicesPojo servicesPojo : serviceList) {
            table.addCell(new Phrase(servicesPojo.getIdservicecategories().getName() + "", font1));
...
//            table.addCell(new Phrase(unitOfMeasurementPojo.getUnitOfMeasurement() + "", font1));
    ...
    public List<ServicesPojo> getServiceList(String searchText ){
        ...
        List<Services> list = new ArrayList<>();
        if (!StringUtils.isEmpty(searchText)) {
            list =servicesRepository .findAllByNameContaining(searchText,sort);
        } else {
...
//            headerRow1.createCell(2).setCellValue("UOM");
            ...
            for (ServicesPojo servicesPojo : servicesPojoList) {
                HSSFRow row = sheet.createRow(++i);
                row.createCell(0).setCellValue(servicesPojo.getIdservicecategories().getName());
...
//                row.createCell( 1 ).setCellValue( list.getCustomers().getTodate() );
    ...
    @Transactional
    public void downloadAppointmentreportPdf(OutputStream outputStream, Date fromdate, Date todate) {
        try {
            Font font1 = new Font(getcustomfont(), 12F, Font.BOLD);
            Document document = new Document(PageSize.A4);
            PdfWriter.getInstance(document, outputStream);
            ...
            Chunk CONNECT1 = new Chunk(new com.lowagie.text.pdf.draw.LineSeparator(1, 100, Color.WHITE, Element.ALIGN_JUSTIFIED, 3f));
            document.add(CONNECT1);
            PdfPTable table = createFirstApntreport(fromdate, todate );
            document.add(table);
            document.add(CONNECT1);
            Paragraph foot = new Paragraph();
            document.add(foot);
    ...
    public PdfPTable createFirstApntreport(Date fromdate, Date todate) {
        Font font1 = new Font(getcustomfont(), 8, Font.NORMAL, Color.BLACK);
        int a = 0;
        PdfPTable tab = new PdfPTable(1);
        Font f = new Font(getcustomfont(), 15, Font.BOLD, Color.WHITE);
        ...
        table.addCell(pc6);
        table.addCell(pc7);
        List<AppointmentPojo> appList =getAppointmentreport(fromdate,todate);
        for (AppointmentPojo list : appList) {
            table.addCell(new Phrase(list.getCustomers().getFirstName() + "", font1));
            table.addCell(new Phrase(list.getAppointmentNo() + "", font1));
            if(list.getCustomers().getEmail()!=null) {
                table.addCell( new Phrase( list.getCustomers().getEmail() + "", font1 ) );
            }else{
                table.addCell( new Phrase( "" ) );
            }
            if(list.getCustomers().getAddress()!=null) {
                table.addCell( new Phrase( list.getCustomers().getAddress() + "", font1 ) );
            }else{
                table.addCell( new Phrase( "" ) );
            }
    ...
    @Transactional
    public void downloadAppointmentreportExcel(OutputStream out,Date fromdate,Date todate) {
        try {
            List<AppointmentPojo> appList =getAppointmentreport(fromdate, todate );
            HSSFWorkbook hwb = new HSSFWorkbook();
            HSSFSheet sheet = hwb.createSheet("First Sheet");

...
import com.hyva.hospital.holistic.respositories.CategoriesRepository;
import com.hyva.hospital.holistic.respositories.ServicesRepository;
...
//            User bshimData = bshimUserService.get(credentials) ;
        ...
        public UserPojo userValidate(@RequestBody UserPojo userPojo)  {

            return userService.userValidate(userPojo);
        }
        @RequestMapping(value = "/submituserdetails", method = RequestMethod.POST,consumes = "application/json", produces = "application/json")
        public Users submituserdetails(@RequestBody UserPojo userPojo,
                                       @RequestParam(value = "type") String type)  throws Exception {

            return userService.submituserdetails(userPojo,type);
        }
    ...
    public ResponseEntity getPaginatedUsersList(@RequestParam(value = "searchText", required = false) String searchText,
        ...
        return ResponseEntity.status(200).body(userService.getPaginatedUsersList(basePojo, searchText));
    }
...
//        }
    ...
    public EntityResponse getCustomers(@RequestParam(value = "userid")Long userid){
        List<CustomersPojo> userPojos = userService.customerList(userid);
...
//    }
    ...
    @RequestMapping(value = "/getprovidersbasedonservice", method = RequestMethod.POST)
    public ResponseEntity getprovidersbasedonservice(@RequestParam(value = "name",required = false) String serviceName) {
        return ResponseEntity.status(200).body(userService.getprovidersbasedonservice(serviceName));
    }
    @RequestMapping(value = "/getservicebasedoncategories", method = RequestMethod.POST)
    public ResponseEntity getservicebasedoncategories(@RequestParam(value = "name") String name) {
        return ResponseEntity.status(200).body(userService.getservicebasedoncategories(name));
    }
...
//    }
    ...
    public ResponseEntity saveCurrentUserDetails(@RequestBody UserPojo userPojo ) throws Exception  {
        ...
        userPojo1 =  userService.SaveCurrentUser(userPojo);
        return ResponseEntity.status(200).body(userPojo1);
    }
    ...
    @RequestMapping(value = "/saveNewAppointment",method = RequestMethod.POST,consumes ="application/json")
         public ResponseEntity saveNewAppointment(@RequestBody AppointmentPojo appointmentPojo,
                                                  @RequestParam (value="userid")Long userId) throws Exception  {
            Appointments master = null;
        master =  userService.SaveAppointment(appointmentPojo,userId);
    ...
    @RequestMapping(value = "/saveOldAppointments",method = RequestMethod.POST,consumes ="application/json")
         public ResponseEntity saveOldAppointments(@RequestBody AppointmentPojo appointmentPojo ,
                                                   @RequestParam (value = "userid")Long userid) throws Exception  {
            Appointments master = null;
        master =  userService.saveOldAppointments(appointmentPojo,userid);
        return ResponseEntity.status(200).body(appointmentPojo);
    }
    ...
    @RequestMapping(value = "/getAppointmentreport", method = RequestMethod.POST,produces = MediaType.APPLICATION_JSON_VALUE)
    public EntityResponse getAppointmentreport(@RequestParam(value = "fromdate") String  fromdate,
        ...
        List<AppointmentPojo> appointmentList = userService.getAppointmentreport(new java.sql.Date(fromdate1.getTime()),
                                                                                 new java.sql.Date(todate1.getTime()));
        return new EntityResponse(HttpStatus.OK.value(), " success", appointmentList);
    }
...
//    }
   ...
   public ResponseEntity getallocatedTimeSlots(@RequestParam(value = "date")String  date) throws ParseException {
    ...
    @RequestMapping(value = "/getNewPatient",method = RequestMethod.POST,produces = "application/json")
    public ResponseEntity getPatient(@RequestParam(value="appointmentId") Long appointmentId) {
            Patient patient=null;
            patient = userService.getPatient(appointmentId);
...
//    }
   ...
   public ResponseEntity getPaginatedCountryList(@RequestParam(value = "type", required = false) String type,
    ...
    public ResponseEntity getPaginatedAppointmentList(@RequestParam(value = "searchText",required = false) String searchText,
        ...
        return ResponseEntity.status(200).body(userService.getPaginatedAppointmentList(basePojo,searchText,userid));
    }

    @RequestMapping(value = "/saveloginStatus",method = RequestMethod.POST,produces = "application/json")
    public ResponseEntity getloginStatus(){
            return ResponseEntity.status(200).body(userService.getloginStatus());
    }
...
//                    appointmentPojo.setServices(services  == null ? null : services.toString());
                    ...
                    if(book_datetime!=null) {
                        ...
                        appointmentPojo.setBook_datetime(new java.sql.Date( new java.util.Date(  ).getTime() ) );
                    }
                    ...
                    userService.SaveAppointment(appointmentPojo,null);
                }
            }
        }catch (Exception e){
            e.printStackTrace();
...
//                    appointmentPojo.setUhid(UHID== null ? null : UHID.toString());
                    ...
                    providersPojo.setPhoneNumber(phoneNumber == null ? null : phoneNumber.toString());
        ...
        try {
            ...
            for (int i = 1; i < sheet.getLastRowNum() + 1; i++) {
                ...
                if(row!=null&&row.getPhysicalNumberOfCells()>0) {
                    ...
                    org.apache.poi.ss.usermodel.Cell duration = row.getCell(1);
                    org.apache.poi.ss.usermodel.Cell price = row.getCell(2);
                    org.apache.poi.ss.usermodel.Cell currency = row.getCell(3);
                    org.apache.poi.ss.usermodel.Cell availabilitiesType = row.getCell(4);
                    org.apache.poi.ss.usermodel.Cell flag = row.getCell(5);
                    org.apache.poi.ss.usermodel.Cell attendantsNumber = row.getCell(6);
                    org.apache.poi.ss.usermodel.Cell description = row.getCell(7);
                    org.apache.poi.ss.usermodel.Cell id_service_categories = row.getCell(8);
                    ServicesPojo servicesPojo = new ServicesPojo();
                    servicesPojo.setName(name == null ? null : name.toString());
                    servicesPojo.setDuration(duration == null ? null :Integer.valueOf(new java.text.DecimalFormat("0").format( duration.getNumericCellValue())));

                    servicesPojo.setPrice(price == null ? null :Integer.valueOf(new java.text.DecimalFormat("0").format( price.getNumericCellValue())));

                    servicesPojo.setCurrency(currency == null ? null : currency.toString());
//                    servicesPojo.setAvailabilities_type(availabilitiesType == null ? null : availabilitiesType.toString());
                    servicesPojo.setAvailabilities_type(availabilitiesType == null ? null : availabilitiesType.toString());
                    servicesPojo.setFlag(flag == null ? null :String.valueOf(flag.toString()) );
//                    servicesPojo.setFlag(flag == null ? null :new DecimalFormat("0").format( flag.getNumericCellValue()));

                    servicesPojo.setAttendants_number(attendantsNumber == null ? null :Integer.valueOf(new java.text.DecimalFormat("0").format( attendantsNumber.getNumericCellValue())));

                    servicesPojo.setDescription(description == null ? null : description.toString());
                    ServiceCategories serviceCategories=categoriesRepository.findByName(id_service_categories.toString());
                    if(serviceCategories!=null) {
                        servicesPojo.setCategoryId(serviceCategories.getId());
                        }
                    userService.SaveServices(servicesPojo);
                }
            }
        }catch (Exception e){
            e.printStackTrace();
...
//                    employeeDTO.setEmployeePhone(empPhone == null ? null :  empPhone.toString());
    ...
    @RequestMapping(path = "/appointmentreportExport", method = RequestMethod.GET)
    public ResponseEntity appointmentreportExport(@RequestParam(value = "type") String type,
                                        @RequestParam(value = "fromDate") java.sql.Date fromdate,
                                                  @RequestParam(value = "toDate") java.sql.Date todate) {
        HttpHeaders headers = new HttpHeaders();
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        if (StringUtils.equalsIgnoreCase(type, "Pdf")) {
            userService.downloadAppointmentreportPdf(outputStream,fromdate,todate);
            headers.add("Content-Disposition", "attachment; filename=\"" + "Appointment.pdf" + "\"");
        } else if (StringUtils.equalsIgnoreCase(type, "Excel")) {
            userService.downloadAppointmentreportExcel(outputStream,fromdate,todate);
...
//                                            @RequestParam(value = "toDate") String todate
    ...
    public ResponseEntity getDeleteUser(@RequestParam(value = "id") Long id) {
    ...
    public EntityResponse getUserList() {
        List<UserPojo> userPojo = userService.getUserList();
        return new EntityResponse(HttpStatus.OK.value(), " success", userPojo);
    }
    @RequestMapping(value = "/verifyotpdetails", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity verifyotpdetails(@RequestParam(value = "phoneno")String phoneno,
                                           @RequestParam(value = "otp") String otp) {
        return ResponseEntity.status(200).body(userService.verifyotpdetails(phoneno,otp));
    }

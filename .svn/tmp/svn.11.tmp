app.controller('appointmentController',
    function($scope, $http, $location, $filter, Notification, ngTableParams,  $timeout, $window, $rootScope) {
        ...
        $scope.loginStatus = function () {
            $http.post($scope.hospitalServerURL + "/saveloginStatus").then(function (response) {
                var data = response.data;
                $rootScope.loginUserId = data.id;
        ...
        $scope.saveServices=function () {
            if($scope.serviceDetailsList==null||$scope.serviceDetailsList==""||angular.isUndefined($scope.serviceDetailsList)){
                Notification.error({message:'Please Select Atleast One service',positionX:'center', delay: 2000})
            }else {
                $http.post($scope.hospitalServerURL + "/saveServiceInv?appointmentId=" + $scope.appointmentId, angular.toJson($scope.serviceDetailsList)).then(function (response) {
                    ...
                    if (data == "") {
                        ...
                        // $scope.printDivA4('receiptPritableArea');
                        $("#add_invoice").modal('hide');
                        $scope.getServicesList();
                        angular.forEach($scope.appointmentList,function (val,key) {
                            if(val.id==$scope.appointmentId){
                                $scope.schedule(val, "print");
                            }
                        })
        ...
        //     $scope.sympton="";
        //     $scope.associatedComplaint="";
        //     $scope.historyOfPatient="";
        //     $scope.familyHistory="";
        //     $scope.medicineHistory="";
        //     $scope.obgHistory="";
        //     $scope.occupationalHistory="";
        //     $scope.bowels="";
        //     $scope.appetite="";
        //     $scope.micturation="";
        //     $scope.sleep="";
        //     $scope.pulse="";
        //     $scope.repositoryRate="";
        //     $scope.bp="";
        //     $scope.temperature="";
        //     $scope.pallor="";
        //     $scope.weight="";
        ...
        $scope.getPaginatedAppointmentList = function (page) {
            ...
            $http.post($scope.hospitalServerURL + '/getPaginatedAppointmentList?searchText=' + $scope.searchText + '&userid='+ $rootScope.loginUserId, angular.toJson(paginationDetails)).then(function (response) {
                var data = response.data;
                console.log(data);
        ...
        $scope.getPatientByName=function(type) {
            $http.post($scope.hospitalServerURL + "/getPatientByName?name=" + type).then(function (response) {
                var data = response.data;
                ...
                document.getElementById('SelectDisease').value=data.sympton;
        ...
        $scope.patientVisit = function (data) {
            ...
            if(data.customers.lastName!=null) {
                $scope.patientName = data.customers.firstName + data.customers.lastName;
            }else{
                $scope.patientName = data.customers.firstName;
            }
            ...
            $scope.sympton="";
            $scope.bowels="";
            $scope.historyOfPresentingComplaints="";
            $scope.obgHistory="";
            $scope.jointMobilit="";
            $scope.differentialDiagnosis="";
            $scope.investigation="";
            ...
            $scope.differentialDiagnosis="";
            $scope.diagnosis="";
            $http.post($scope.hospitalServerURL + "/getNewPatient?appointmentId="+data.id).then(function (response) {
                var data = response.data;
                $scope.id=data.id;
                ...
                $scope.obgHistory = data.obghistory;
                $scope.differentialDiagnosis = data.differentialDiagnosis;
                $scope.jointMobilit = data.jointmobility;
                $scope.investigation=data.investigation;
                $scope.treatment=data.treatment;
                $scope.repositoryRate=data.repiratoryrate;
                if(data.sympton!=undefined) {
                    document.getElementById('SelectDisease').value = data.sympton;
                }
        ...
        $scope.saveOldAppointments = function() {
            if ($scope.firstname == "" || $scope.firstname == null || angular.isUndefined($scope.firstname)) {
                ...
                $http.post($scope.hospitalServerURL + "/saveOldAppointments?&userid="+$rootScope.loginUserId, angular.toJson(saveAppointmentDetails)).then(function (response) {
                    var data = response.data;
                    $scope.getPaginatedAppointmentList();
        ...
        $scope.saveAppointment = function () {
            ...
            // }
                ...
                var SelectAppointmentDisease=$('#SelectAppointmentDisease').val();
                var saveAppointmentDetails;
                saveAppointmentDetails = {
                    id: $scope.id,
                    serviceName: $scope.serviceName,
                    ...
                    city: $scope.city,
                    uhid: $scope.uhid,
                    cheifComplaints: SelectAppointmentDisease,
                    countryName: $scope.countryName,
                    stateName: $scope.stateName,
                    starttime: $scope.appntTime,
                    notes: $scope.notes,
                ...
                $http.post($scope.hospitalServerURL + "/saveNewAppointment?userid="+ $rootScope.loginUserId, angular.toJson(saveAppointmentDetails)).then(function (response) {
                    var data = response.data;
                    if (data == "") {
                        Notification.error({message: ' Already exists', positionX: 'center', delay: 2000});
                    }
        ...
        $scope.schedule = function (data,type) {
        ...
        $scope.view = function (data) {
            ...
            if(customers.lastName!=null) {
                ...
                $scope.firstname = data.customers.firstName;
            }
            $scope.emailId =data.customers.email;
        ...
        $scope.addAppointment = function () {
            ...
            $scope.getServicesList();
            // $scope.categoryId = "Consultation";
            // $scope.getservicebasedoncategories($scope.categoryId);
            // $scope.serviceName = "Consultation";
            // $scope.getprovidersbasedonservice($scope.serviceName);
            $scope.date = new Date();
        ...
        $scope.SaveProviders = function (flagName) {
            if (angular.isUndefined($scope.firstName) || $scope.firstName == ''||$scope.firstName== null) {
                Notification.error({message: ' Please Enter the First Name', positionX: 'center', delay: 2000});
            }
            else if (angular.isUndefined($scope.mobileNumber) || $scope.mobileNumber == ''||$scope.mobileNumber== null) {
                Notification.error({message: ' Please Enter Mobile Number', positionX: 'center', delay: 2000});
            }
            else {
                ...
                $http.post($scope.hospitalServerURL + "/saveProviderDetails", angular.toJson(saveProviderDetails)).then(function (response) {
                    ...
                    if (data == "") {
                        ...
                        $scope.getprovidersbasedonservice();
                        $scope.getservicebasedoncategories();
                        $("#addon_providers").modal('hide');
                        if($scope.operation=='Edit'){
                            Notification.success({
        ...
        $scope.savePatient=function (data) {
            var SelectDisease=$('#SelectDisease').val();
            ...
            savePatientDetails = {
                ...
                duration : $scope.duration,
                morning : $scope.morning,
                repiratoryrate : $scope.repositoryRate,
                afternoon : $scope.afternoon,
                night : $scope.night,
                afterFood : $scope.afterFood,
                afterFood1 : $scope.afterFood1,
        ...
        $scope.addNewPatient = function () {
            if ($scope.categoryId == "" || $scope.categoryId == null || angular.isUndefined($scope.categoryId)) {
                Notification.error({message: ' Please Select Category', positionX: 'center', delay: 2000});
            }
            else if(($scope.serviceName == "" || $scope.serviceName == null || angular.isUndefined($scope.serviceName))&& $scope.categoryId !="Consultation" ){
                Notification.error({message: ' Please Select Service', positionX: 'center', delay: 2000});
            }
            // else if ($scope.providerName == "" || $scope.providerName == null || angular.isUndefined($scope.providerName)) {
            //     Notification.error({message: ' Please Select Providers', positionX: 'center', delay: 2000});
        ...
        $scope.getprovidersbasedonservice=function () {
            $http.post($scope.hospitalServerURL + "/getprovidersbasedonservice?name=" +$scope.serviceName).then(function (response) {
                var data = response.data;
                $scope.providersList=data;
            });
        ...
        $scope.getservicebasedoncategories=function () {
            $http.post($scope.hospitalServerURL + "/getservicebasedoncategories?name=" +$scope.categoryId).then(function (response) {
                var data = response.data;
                $scope.servicesList=data;

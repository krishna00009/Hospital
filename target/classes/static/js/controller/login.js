...
app.controller('loginController',
    function($scope, $http, $location, $filter, Notification, ngTableParams,  $timeout, $window, $rootScope) {
        ...
        $scope.signuservalidate = function () {
            $scope.userText = "";
        ...
        $scope.saveuservalidate = function () {
            ...
            saveUserDetails = {
                ...
                // id:$scope.id,
                phoneNumber: $scope.TelephoneNumberText,
        ...
        $scope.submituserdetails = function (type) {
            if ($scope.TelephoneNumberText == null || $scope.TelephoneNumberText == "" || angular.isUndefined($scope.TelephoneNumberText)) {
                Notification.error({message: 'Please Enter Phone Number', positionX: 'center', delay: 2000})
            } else if ($scope.userText == null || $scope.userText == "" || angular.isUndefined($scope.userText)) {
                Notification.error({message: 'Please Enter Name', positionX: 'center', delay: 2000})
            } else {
                var saveUserDetails;
                saveUserDetails = {
                    // password: $scope.pswdText,
                ...
                $http.post("/hospital/submituserdetails?type=" + type , angular.toJson(saveUserDetails)).then(function (response) {
                    var data = response.data;
                    console.log("login page :")
                    console.log(data);
                    $rootScope.userName = data.username;
                    // $scope.sendOtp(data,"phoneNumber");
                    // $scope.getUserList();
                    if (data == "") {
        ...
        $scope.verifyotpdetails = function () {
            if ($scope.otp == null || $scope.otp == "" || angular.isUndefined($scope.otp)) {
                Notification.error({message: 'Please Enter OTP', positionX: 'center', delay: 2000})
            } else {
                var saveOtpDetails;
                saveOtpDetails = {
                    otp: $scope.otp,
                ...
                $http.post("/hospital/verifyotpdetails?otp=" + $scope.otp + '&phoneno=' + $scope.TelephoneNumberText, angular.toJson(saveOtpDetails)).then(function (response) {
                    var data = response.data;

                    if (data == "") {
                        Notification.error({message: 'Please Enter Valid OTP', positionX: 'center', delay: 2000});
                    } else {
                        $scope.removepassword();
                        $("#add_otp_modal").modal('hide');
                        $("#add_password_modal").modal('show');
                        Notification.success({
                            message: 'User Created  successfully,Please Keep Password',
                            positionX: 'center',
                            delay: 2000
                        });
                    }
        ...
        $scope.savepassword = function (type) {
            if ($scope.pswdText == null || $scope.pswdText == "" || angular.isUndefined($scope.pswdText)) {
                Notification.error({message: 'Please Enter Password', positionX: 'center', delay: 2000});
            }
            else if ($scope.retypepswrd == null || $scope.retypepswrd == "" || angular.isUndefined($scope.retypepswrd)) {
                Notification.error({message: 'Please Enter Retype Password', positionX: 'center', delay: 2000});
            }
            else if ($scope.retypepswrd != $scope.pswdText) {
                ...
                saveUserDetails = {
                    ...
                    password: $scope.pswdText,
                    phoneNumber: $scope.TelephoneNumberText,
                    retypepassword: $scope.retypepswrd,
                };
                $http.post("/hospital/submituserdetails?type=" + type, angular.toJson(saveUserDetails)).then(function (response) {
                    var data = response.data;
                    if(data == null){

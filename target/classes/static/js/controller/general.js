app.controller('generalController',
    function($scope, $http, $location, $filter, Notification, ngTableParams,  $timeout, $window, $rootScope) {
        ...
        $scope.saveGeneralSettingDetails = function () {
            ...
            else if(angular.isUndefined($scope.CompanyEmail) || $scope.CompanyEmail == ''|| $scope.CompanyEmail == null){
                Notification.warning({message: 'Enter valid EmailId', positionX: 'center', delay: 2000});
            }
            else if(angular.isUndefined($scope.CompanyLink) || $scope.CompanyLink == '' || $scope.CompanyLink ==null){
                Notification.warning({message: 'Company link can not be Empty', positionX: 'center', delay: 2000});
            }
            else if(angular.isUndefined($scope.DateFormat) || $scope.DateFormat == ''|| $scope.DateFormat == null){

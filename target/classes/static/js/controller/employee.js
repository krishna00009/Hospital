...
app.controller('employeeCtrl',
    function ($scope, $http, $location, $filter, Notification, ngTableParams, $timeout) {
        ...
        $scope.saveNewEmployee = function () {
            if (angular.isUndefined($scope.EmployeeNameText) || $scope.EmployeeNameText == ''|| $scope.EmployeeNameText==null) {
                Notification.warning({message: 'Please Enter Employee Name', positionX: 'center', delay: 2000});
            }
            // else if (angular.isUndefined($scope.EmployeeCodeText) || $scope.EmployeeCodeText == '' || $scope.EmployeeCodeText==null ) {
            //     Notification.warning({message: 'Please Enter Employee Code', positionX: 'center', delay: 2000});
            // }
            else if (angular.isUndefined($scope.EmployeePhoneText) || $scope.EmployeePhoneText == ''|| $scope.EmployeePhoneText==null ) {
                Notification.warning({message: 'Please Enter Phone Number', positionX: 'center', delay: 2000});
            }
            else if (angular.isUndefined($scope.dt) || $scope.dt == ''|| $scope.dt==null ) {
                Notification.warning({message: 'Please Enter DOB', positionX: 'center', delay: 2000});
            }
            else if (angular.isUndefined($scope.dt1) || $scope.dt1 == '' || $scope.dt1==null ) {
